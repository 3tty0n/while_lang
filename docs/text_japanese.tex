...
配布するソースコードは大部分が実装されていますが、パーサーが足し算と $<$ 以外の演算を認識
しません。まず、引き算、かけ算、割り算、$>$、$<=$、$>=$ 、$==$ などの演算を実装してくださ
い。また、パーサは \verb|begin| -- \verb|end| 文、そして \verb|while| 文を認識しません。
\verb|parser.mly| と \verb|lexer.mll| を改造し、上記の文を認識できるよう改良してください。
